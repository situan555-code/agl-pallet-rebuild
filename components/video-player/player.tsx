...
import { VideoMediaSessionController } from "@/components/video-player/components/media-session-controller"
...
import { QuietControls } from "@/components/video-player/components/quiet-controls"
...
export interface VideoPlayerProps extends PlaybackSourceControllerProps<VideoPlayerAsset> {
  ...
  /**
   * `full` is the installed Limeplay control bar.
   * `quiet` is play/pause and mute only, for a silent hero loop.
   * The timeline stays unmounted so elapsed timestamps are not rendered.
   *
   * @default "full"
   */
  chrome?: "full" | "quiet"
      ...
      <MediaProvider debug={debug}>
        ...
        >
          ...
          <Layout.PlayerContainer className="size-full min-h-0">
            <FallbackPoster>
              {chrome === "full" ? <LimeplayLogo /> : null}
            </FallbackPoster>
            <CurrentAssetMedia {...(mediaProps ?? {})} />
            {children}
            {chrome === "quiet" ? <QuietControls loop={Boolean(mediaProps?.loop)} /> : null}
            {chrome === "full" ? (
              <>
                ...
                >
                  <TopOverlayContainer
                    className="
                      px-[5%] pt-[clamp(0.75rem,7svh,4rem)] transition-[transform,opacity] duration-300 ease-[cubic-bezier(0.25,0.46,0.45,0.94)]
