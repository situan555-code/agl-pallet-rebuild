...
- **Not changed.** `scripts/tokens.js`, `scripts/copy-verbatim.js`, `scripts/height.js`, `pages.json`, audit thresholds, package name, canonical host, noindex rule, sell/recycle 404. `npm run tokens` still fails the omitted placeholders. `npm run copy-verbatim` still fails only `Paid on agreed terms. — {{TBD-CARRIER-TERMS}}` on `/partners/carriers`. Height was not re-run.
