...
Root-relative internal links (FormSubmit `_next` no longer hardcodes the demo host). Footer address placeholder removed; phone and email kept. Organization JSON-LD: name, url, logo, telephone, email, areaServed US, sameAs for the live LinkedIn and Facebook profiles only. No LocalBusiness, no street address. Build guard `scripts/placeholder-guard.js` fails `npm run build` if rendered HTML contains `{{`, `TBD`, or a `vercel.app` URL. Byline stays "Published by the AGL Pallet brokerage desk" while `authors.json` is placeholders; Article JSON-LD omits author and reviewedBy until real names exist. Changelog on each guide from Waves 1–4. `scripts/tokens.js` not modified (it will fail the routes that no longer show placeholder tokens). Sell/recycle still omitted. See DECISIONS.md §2.

### 2026-09-24 Resource Library §3 content accuracy
Short answers for GMA, weight, truckload, and ISPM-15 rewritten to 40–60 words with the named figures. Hub and related cards use the first complete sentence. ISPM-15 stamp section: hyphen rule (US-1234) with the APHIS/CBP January 1, 2026 cite, plus DUN on the stamp list and export checklist. GMA notes the 2020 Consumer Brands Association rename from the association's own launch release. Empty AGL weigh and price tables hidden until `content/resources/data/agl-weighed.json` and `agl-indicative-bands.json` have values; one method sentence, no date, no invented figures. Cost-per-trip: optional one-way price, comparison row, labeled worked example on first load (sample dollars, not an AGL price). Glossary: per-term anchors already present; added DUN, SF, ALSC, butted boards, pallet exchange; first mentions linked from the guides. Standards: ANSI MH1-2021, Uniform Standard 2025, ASTM D1185-98a(2025). Shared planning default 6 in / 37 lb; estimator notes its own calculation may differ. Truckload table shows interior height and the empty-stack difference between 40 ft standard and high-cube. §4 question pages shipped on this branch (13 of 15; MB and plastic-vs-wood skipped — see DECISIONS.md §4). §5–§6 not started. See DECISIONS.md §3.

### 2026-09-24 Resource Library §5 downloads and stamp decoder
`/resources/downloads/` index, pallet size chart page + PDF, one-page ISPM 15 stamp cheat sheet page + PDF, and `/resources/heat-treated-pallets-ispm-15/stamp-decoder/`. PDFs and HTML read the guide JSON (size tables by object identity; stamp fields, treatment table, hyphen, DUN from the heat-treatment guide). Decoder validates the hyphen rule and explains each field. Empty stamp-photo slot, no stand-in image. WebApplication JSON-LD. Hub, nav, sitemap via SITE_URL. §4 thin skips unchanged (MB meaning, plastic vs wood). §6 not started. Sell/recycle still omitted. See DECISIONS.md §5.
...
`npm run color`, `npm run banned-words`, and `npm run audit` pass. Thresholds unchanged. Legacy SVG fill is `#162619`. SPEC-route banned-word hits rephrased in brokerage voice and mirrored in SPEC_V1.md. Scanner ignores DOCTYPE, React comments, and Tailwind `!class`. Audit: perf 98–100, LCP 1812–2331 ms, CLS 0, axe serious/critical 0 on the six pages.json routes. Header is server-rendered; carousel JS loads after first paint. Wave B still open: tokens.js (placeholders not rendered), copy-verbatim only `/partners/carriers` TBD-CARRIER-TERMS, height.js not re-run. `{{TBD-*}}` not put back into HTML. Sell/recycle still 404. No HowTo. No `llms.txt`. See DECISIONS.md Wave A.
